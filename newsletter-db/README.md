# Database Backend for the Guestbook user-profile Microservice
The user-profile-backend requires a persistent database backend. The user-profile-backend and the 
user-profile-database belong together but are deployed different depending on the envronment (local or within 
a kubernetesccluster). In the following we describe both procedure. 
...
```
$ psql -h localhost -p 5432 -U user -d subscription    ## pssword: password
